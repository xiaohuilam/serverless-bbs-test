...
const AdminLayout = ({ children }: { children: React.ReactNode }) => {
    ...
    const renderSidebarNav = () => {
        if (location.pathname.startsWith('/admin/settings')) {
            return (
                 <ul>
                    <li><Link to="/admin/settings" className="block py-2 text-sm text-gray-700 hover:bg-gray-200">站点信息</Link></li>
                    <li><Link to="/admin/settings/registration" className="block py-2 text-sm text-gray-700 hover:bg-gray-200">注册设置</Link></li>
                </ul>
            );
        }
         if (location.pathname.startsWith('/admin/users')) {
            return (
                 <ul>
                    <li><Link to="/admin/users" className="block py-2 text-sm text-gray-700 hover:bg-gray-200">用户管理</Link></li>
                    <li><Link to="/admin/users/groups" className="block py-2 text-sm text-gray-700 hover:bg-gray-200">用户组</Link></li>
                </ul>
            );
        }
        // 默认显示“首页”的侧边栏
        return (
            <ul>
                <li><Link to="/admin/dashboard" className="block py-2 text-sm text-gray-700 bg-gray-200">系统信息</Link></li>
            </ul>
        );
    }
